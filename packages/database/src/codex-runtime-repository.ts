...
export class PostgresCodexRuntimeRepository implements CodexRuntimePersistence {
  ...
  async loadBindingForClaim(guard: CodexRuntimeGuard): Promise<CodexClaimContextBinding> {
    ...
    return this.tx(async (client) => {
      ...
      [guard.jobId,guard.workerId,guard.claimId,guard.fencingToken,guard.leaseGeneration,guard.claimedJobVersion])).rows[0];
  ...
  async authorizeStart(input: Parameters<CodexRuntimePersistence["authorizeStart"]>[0]): Promise<CodexStartDecision> {
    ...
    return this.tx(async (client) => {
      ...
      if (existing) {
        ...
        if (existing.worker_id === input.guard.workerId && existing.claim_id === input.guard.claimId && Number(existing.fencing_token) === input.guard.fencingToken && Number(existing.lease_generation) === input.guard.leaseGeneration) {
      ...
      const inserted = (await client.query<RunRow>(`INSERT INTO builder.codex_exec_runs(
          project_id,job_id,run_id,state,prompt_sha256,cli_version,worker_id,claim_id,lease_generation,fencing_token,claimed_job_version,started_at
        ) VALUES($1,$2,$3,'DISPATCHED',$4,$5,$6,$7,$8,$9,$10,clock_timestamp()) RETURNING ${runColumns}`,
      [projectId,input.guard.jobId,input.runId,input.promptSha256,input.cliVersion,input.guard.workerId,input.guard.claimId,input.guard.leaseGeneration,input.guard.fencingToken,input.guard.claimedJobVersion])).rows[0]!;
  ...
  private async finish(
    ...
    return this.tx(async (client) => {
      ...
      [projectId,guard.jobId,runId,state,output===undefined?null:JSON.stringify(output),errorCode??null,policyEvent??null,threadId??null,model??null,usage===undefined?null:JSON.stringify(usage),promptSha256,guard.workerId,guard.claimId,guard.leaseGeneration,guard.fencingToken,guard.claimedJobVersion])).rows[0];
      if (!row) throw new AgentJobLeaseLostError();
  ...
  private async lockOwnedAfterDispatch(
    ...
    const row = (await client.query<{project_id:string}>(`SELECT job.project_id FROM builder.background_jobs job
      ...
      WHERE job.id=$1 AND job.lease_owner=$2 AND job.claim_idempotency_key=$3 AND job.fencing_token=$4
        ...
        AND runtime.runtime_started_at IS NULL AND runtime.runtime_snapshot='{}'::jsonb
        AND runtime.runtime_fencing_token=0 AND runtime.result_id IS NULL
        AND runtime.workload_id IS NULL AND runtime.process_identity IS NULL
        AND (
      ...
      FOR UPDATE OF job`,
    [guard.jobId,guard.workerId,guard.claimId,guard.fencingToken,guard.leaseGeneration,guard.claimedJobVersion,allowCancelling])).rows[0];
    if (!row) throw new AgentJobLeaseLostError();
