...
export class PersistentWorkflowRepository implements WorkflowRepository {
  ...
  authorizeJobWork(request:OwnedJobRequest):Promise<WorkflowJob>{return this.mutateOwned(request,["CLAIMED"],memory=>memory.authorizeJobWork(request));}
  heartbeatJob(request:HeartbeatJobRequest):Promise<WorkflowJob>{return this.mutateOwned(request,["CLAIMED"],memory=>memory.heartbeatJob(request));}
  ...
  private mutateOwned<T>(request:OwnedJobRequest,allowedStatuses:readonly WorkflowJob["status"][],operation:(memory:InMemoryWorkflowRepository)=>Promise<T>):Promise<T>{
  ...
  private async load(projectId:string):Promise<{memory:InMemoryWorkflowRepository;snapshot:string;storageVersion:number}|null>{
    ...
    const memory=new InMemoryWorkflowRepository(this.memoryOptions); memory.importPersistentState(stored.snapshot);
  ...
  private async mutate<T>(projectId:string,operation:(memory:InMemoryWorkflowRepository)=>Promise<T>):Promise<T>{
    for(let attempt=0;attempt<this.maxCasRetries;attempt++){
      ...
      if(snapshot===loaded.snapshot){if(failure)throw failure;return result;}
      const capability=await this.options.projectCapability(projectId,"workflow_state:append");
      if(await this.options.database.compareAndSwapWorkflowState(capability,loaded.storageVersion,snapshot,loaded.memory.exportPersistenceProjection(projectId))){if(failure)throw failure;return result;}
    }
