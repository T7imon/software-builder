...
export interface CreateAgentVersionInput extends AgentVersionContent { readonly agentKey: string; }
...
export class AgentRegistryService {
  ...
  createDefinition(capability:ProjectCapability,input:CreateAgentDefinitionInput){validateContent(input);return this.repository.createDefinition(capability,input);}
...
export class PostgresAgentRegistryRepository implements AgentRegistryRepository {
  ...
  createDefinition(capability:ProjectCapability,input:CreateAgentDefinitionInput){return this.transaction(capability,"agent_registry:append",async session=>{const row=(await session.query<RegistryRow>(`INSERT INTO builder.agent_registry_versions(agent_id,agent_key,display_name,role,description,version,revision,status,instructions,allowed_capabilities,forbidden_capabilities,model_config,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,'DRAFT',$8,$9,$10,$11,$12) RETURNING ${columns}`,values(input,input.agentId??randomUUID()))).rows[0]!;return mapRow(row);});}
  createVersion(capability:ProjectCapability,input:CreateAgentVersionInput){return this.transaction(capability,"agent_registry:append",async session=>{await session.query("SELECT pg_advisory_xact_lock(hashtextextended($1,0))",[`agent-registry:${input.agentKey}`]);const prior=(await session.query<{agent_id:string;role:AgentRole}>("SELECT agent_id,role FROM builder.agent_registry_versions WHERE agent_key=$1 ORDER BY version LIMIT 1",[input.agentKey])).rows[0];if(!prior)throw new Error("Agentendefinition ist unbekannt.");if(prior.role!==input.role)throw new Error("Die Rolle eines agentKey ist unveraenderlich.");const row=(await session.query<RegistryRow>(`INSERT INTO builder.agent_registry_versions(agent_id,agent_key,display_name,role,description,version,revision,status,instructions,allowed_capabilities,forbidden_capabilities,model_config,created_by) VALUES($1,$2,$3,$4,$5,$6,$7,'DRAFT',$8,$9,$10,$11,$12) RETURNING ${columns}`,values(input,prior.agent_id))).rows[0]!;return mapRow(row);});}
