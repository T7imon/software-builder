import { FakeAgentRuntime, SchemaValidationError, type AgentRuntime, type RuntimeStatus, type RuntimeStore } from "@software-builder/agent-runtime";
...
export interface JobProcessorOptions { readonly retryDelayMs?:number;readonly runtimeFactory?:(store:RuntimeStore)=>AgentRuntime;readonly afterRuntimePersisted?:()=>void|Promise<void>; }
...
export class AgentJobProcessor {
  ...
  private async commitCancellation(store:PostgresRuntimeStore,status:RuntimeStatus|undefined){if(!status?.terminal||status.result?.status!=="CANCELLED")throw new Error("RUNTIME_CANCEL_NOT_CONFIRMED");await store.persistProgress(status);await this.repository.confirmCancelled(store.guard(),status.result,this.messageId(store.claim,"cancelled"));}
