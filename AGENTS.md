...
## 9. Builder-V1-Entwicklungsprofil fuer Worker und Runtime

Der Meilenstein `WORKER_FAKE_RUNTIME_MVP` ist ausschliesslich `DEVELOPMENT_ONLY`. Er verwendet nur synthetische Testdaten, `FakeAgentRuntime` und standardmaessig genau einen lokalen Worker. Echte Codex-Agenten, GitHub-Schreibzugriffe oder automatisches Merge, automatische Projektausfuehrung, Deployment und Production sind in diesem Meilenstein verboten. Eine Freigabe dieses Meilensteins ist nur eine lokale Komponentenfreigabe und keine Release-Candidate- oder Produktionsfreigabe.

Cancellation und Completion werden fuer den lokalen Fake-Scope am autoritativen persistenten Jobzustand atomar linearisiert. Ein nachweislich noch nicht gestarteter Fake-Job darf nur auf Grundlage des persistenten Job-/Outbox-Zustands atomar abgebrochen werden. Bei einem laufenden Fake-Job darf `CANCELLED` nur nach bestaetigtem, an den aktuellen Job-/Run-/Claim-Kontext gebundenem FakeRuntime-Cancel committed werden. Fehlerhafte, abgelehnte, zeitlich unklare, nicht nachweisbare oder stale Cancels bleiben fail-closed `CANCEL_STUCK` beziehungsweise blockieren das Gate; sie duerfen nie als `CANCELLED` oder `SUCCEEDED` ausgegeben werden. Wurde Completion zuerst atomar committed, bleibt `SUCCEEDED` bestehen und der Cancel wird als zu spaet abgelehnt. Wurde Cancellation zuerst atomar committed, darf eine spaetere Completion nicht als Erfolg publiziert werden.

Der freigegebene Vertrag `CANCELLATION-CONTRACT-DECISION-01` bleibt unveraenderte Zielarchitektur. Seine vollstaendige Real-Runtime-Umsetzung sowie reale Attestation, externe Statusabfrage, mehrprozessfaehige finale Reconciliation, Completion-ID-Hardening, echte Worker-/Prozessidentitaet und Provider-/Credential-Widerruf gehoeren zum zwingenden, noch nicht bestandenen Meilenstein `REAL_RUNTIME_HARDENING`.

`REAL_RUNTIME_HARDENING` muss fail-closed abgeschlossen sein, bevor `AGENT_RUNTIME=codex`, schreibende echte Codex-Executors, automatische GitHub-Aenderungen, `RELEASE_CANDIDATE` oder `PRODUCTION` zulaessig werden. Sein technischer Abschluss ist notwendig, aber nicht allein hinreichend: alle zugeordneten Security-, Legal-, Provider-, GitHub-, Release- und Owner-Gates bleiben zusaetzlich bindend. Production deployment bleibt fuer Builder V1 `DISABLED`.
