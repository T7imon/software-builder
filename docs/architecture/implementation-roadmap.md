...
5. Security, Legal, and evidence holds prevent exit.
6. Implementation work is exactly one task per workflow and one source writer per project.
7. A milestone never changes the permanent production prohibition.
...
- three-repair transaction and manual stop.
...
- concurrency/crash/restore cannot create repair ordinal 4;
- critical, unclassified, `BLOCK`, `COUNSEL_REQUIRED`, unmet requirement, and stale evidence fail closed;
- after three failures only D-017 manual options are available.
...
| D-004 | Whether writer lease covers QA | Cover all source writers; QA Reviewer remains RO | Architect, QA, Security | M-001 |
...
| D-013 | Repair actor, QA write mode, write handoff, review independence | Executor normally repairs; QA Writer explicit lease and cannot QA-review own revision | Owner, Architect, QA | M-006/M-007 |
...
| D-017 | Manual options after three failed repairs and resume semantics | Never reset counter; abandon, new scoped task, or reviewed external remediation | Owner, Product, QA | M-007 |
...
| D-004 | `TECHNICAL-DECIDED`: The exclusive writer lease covers Executor and QA Writer. QA Reviewer remains read-only. | Prevents revision races and review invalidation. | QA repair waits for Executor handoff. | The lease service can later support branches, but the canonical promotion remains serialized. |
...
| D-013 | `TECHNICAL-DECIDED`: Executor performs automatic repairs. QA may write only after an explicit owner/workflow assignment, exclusive lease transfer, and new revision; that QA identity cannot review its own revision. | Clear independence with a controlled emergency repair path. | QA intervention is slower and needs a handoff. | Add separate human QA identities later without changing self-review denial. |
...
| D-015 | `CONDITIONAL`: Legal verifies `PASS_WITH_REQUIREMENTS`. Qualified human counsel evidence is separately encrypted; counsel closes only `CounselCase`, then Legal issues a successor assessment. Automated continuation remains held during counsel. | Preserves confidentiality and prevents an agent from acting as counsel. | Manual cost and delay; exact counsel qualification/evidence process remains Legal-gated. | Integrate a future counsel workflow/provider behind the same case and successor-assessment model. |
...
| D-017 | `TECHNICAL-DECIDED`: After three failed repairs the owner may abandon the task, create a new scoped task/baseline, or import a manual remediation as a new revision. The old counter is never reset and all eight obligations rerun. | Prevents disguised fourth repairs while preserving recovery choices. | Manual recovery takes longer and can require replanning. | Add approved manual-remediation sources later; history remains immutable. |
...
| D-031 | `TECHNICAL-DECIDED`: Run trusted quality checks first. If the revision remains eligible, run QA, Reviewer, Security, and Legal in parallel on the same read-only digest. Any source, manifest, policy, or material evidence change invalidates affected results; source change reruns all eight. | Reduces wasted reviewer work while preserving independence and freshness. | Parallel reviews consume resources and may produce conflicting requirements needing reconciliation. | Add staged review policies later; digest binding and no self-review remain invariant. |
| D-032 | `TECHNICAL-DECIDED`: Planning `BLOCK`, `COUNSEL_REQUIRED`, critical/unclassified Security, or unresolved evidence prevents approval, workspace creation, automatic execution, and publication. Only read-only analysis, counsel work, or a narrowly authorized remediation workflow may continue. | Safest fail-closed rule and consistent with current Legal/Security reviews. | Conservative holds can delay harmless work. | Add versioned scoped-remediation policies only after Security/Legal approval; no owner waiver for binding holds. |
